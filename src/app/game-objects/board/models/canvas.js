...
export default class Canvas extends GameObject {
    constructor({ inContainer = true, ...props }) {
        ...
        this.getShape = ({ left = 0, top = 0, width = PlayArea.squareSize, height = PlayArea.squareSize, thickness = PlayArea.thickness, offsetShadow = PlayArea.offset.shadow }) => {
        ...
        this.draw = draw[shape]?.(this.ctx);
    }
    set dimensions({ left, ...dim }) {
        this.style = dim;
